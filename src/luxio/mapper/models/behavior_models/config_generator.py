...
class ConfigGenerator:
    ...
    def config_feature_select(self, df):
        ...
        X = df[features]
        y = df[self.stress_test_output_vars]['read_bw']
        param_grid = {
            'n_features': [8],
        ...
        model = RandomForestReducer(features, max_depth=5, n_estimators=100)
        model.fit(X,y)
        self.feature_selector_ = model
        self.named_feature_importances_ = model.sorted_features_

        print(f"Error: {RelativeErrorMetric().score(model.predict(X),y)}")
